...
const CreateEpisode = ({ setUser }) => {
  ...
  useEffect(() => {
    ...
    const fetchOptionsAndPricings = async () => {
      const optionsBdd = await axios.get(
        `${process.env.REACT_APP_URL}/api/options`
      );

      const pricingsBdd = await axios.get(
        `${process.env.REACT_APP_URL}/api/pricings`
      );
      setOptions(optionsBdd.data);
      setPricings(sortByKey(pricingsBdd.data, "nbMotsInf"));
      setIsLoading(false);
    };

    fetchOptionsAndPricings();
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      episodeCommand.status = "0 - Brouillon";
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_URL}/api/create-episode`,
        episodeCommand,
        {
